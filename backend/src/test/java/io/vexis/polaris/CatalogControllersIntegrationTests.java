...
class CatalogControllersIntegrationTests {
    ...
    void shouldPerformCrudForGiftStatusEndpoint() throws Exception {
        String createResponse = mockMvc.perform(post("/gift-status")
                        .contentType(MediaType.APPLICATION_JSON)
                        .content("""
                                {"name":"wrapped"}
                                """))
        ...
        mockMvc.perform(get("/gift-status"))
                .andExpect(status().isOk())
                .andExpect(jsonPath("$[*].name", hasItem("WRAPPED")));

        mockMvc.perform(patch("/gift-status/{id}", id)
                        .contentType(MediaType.APPLICATION_JSON)
                        .content("""
                                {"name":"shipped"}
                                """))
        ...
        mockMvc.perform(delete("/gift-status/{id}", id))
                .andExpect(status().isOk());
    }
