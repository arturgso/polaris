...
public class StatusController {
  ...
  public ResponseEntity<MetadataDTO> list() {
    var giftStatuses =
        Arrays.stream(GiftStatus.values())
            .map(status -> new StatusDTO(status.name(), status.getColor()))
            .toList();

    var shoppingItemStatuses =
        Arrays.stream(ShoppingItemStatus.values())
            .map(status -> new StatusDTO(status.name(), status.getColor()))
            .toList();

    return ResponseEntity.ok(new MetadataDTO(giftStatuses, shoppingItemStatuses));
  }
}
